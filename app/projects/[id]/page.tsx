...
import ProjectRecentMovements from "@/components/ui/payments/RecentMovments"; // <-- NEW
...
interface ProjectStats {
  balance: { total: number; spent: number; remaining: number };
  loans: { total: number; spent: number; remaining: number };
  investments: { total: number; spent: number; remaining: number };
}
...
export default function ProjectDetailPage() {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`/api/projects/${projectId}/dashboard`, { cache: "no-store" });
      if (response.ok) {
        const data = await response.json();
        const transformedStats: ProjectStats = {
          balance: {
            total: data.breakdown?.bySource?.BALANCE || 0,
            spent: Math.abs(data.totals?.expense || 0),
            remaining:
              (data.breakdown?.bySource?.BALANCE || 0) - Math.abs(data.totals?.expense || 0),
          },
          loans: {
    ...
    <BackgroundLayout>
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          {/* Repay */}
          <button
            onClick={() => router.push(`/projects/${projectId}/actions/repayment`)}
            className="group bg-white/5 backdrop-blur-xl border border-white/10 p-6 hover:bg-blue-500/10 hover:border-blue-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/20 hover:scale-105"
          >
            <div className="flex items-center justify-center mb-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm font-semibold">
                  ...
                  <span className="text-emerald-400">
                    ${stats?.balance.remaining.toLocaleString() || "0"}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm font-semibold">
                  ...
                  <span className="text-orange-400">
                    ${stats?.loans.remaining.toLocaleString() || "0"}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-red-400">
                    ${stats?.investments.spent.toLocaleString() || "0"}
                  </span>
                </div>
                <div className="flex justify-between text-sm font-semibold">
                  <span className="text-slate-300">Remaining:</span>
                  <span className="text-purple-400">
                    ${stats?.investments.remaining.toLocaleString() || "0"}
                  </span>
                </div>
              </div>
            </div>
        ...
        <ProjectRecentMovements projectId={projectId} />
      </div>
